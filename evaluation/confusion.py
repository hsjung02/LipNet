...
def get_phonems(word):
    return [str(phoneme).translate(None,string.digits) for phoneme in alphabet[word][0]] #phoneme:음소
...
def get_confusion_matrix(y_true, y_pred, labels, func):
    ...
    for i in range(0,len(y_true)):
        ...
        for j in range(0,len(words_true)):
            ...
            try:
                confusion_matrix = np.add(confusion_matrix,metrics.confusion_matrix(phonemes_true,phonemes_pred, labels=labels))
            except:
                continue
...
for k in range(0,len(SCENARIOS)):
    ...
    confusion_matrix = get_confusion_matrix(y_true,y_pred,_labels,get_phonems)

    plt.figure()
    plot_confusion_matrix(confusion_matrix, classes=labels, normalize=True, title=_name)
...
plt.figure()
plot_confusion_matrix(confusion_matrix, classes=viseme_labels, normalize=True,title=viseme_name)

savepath = os.path.join('confusion',viseme_name+'.png')
print(savepath)
plt.savefig(savepath, bbox_inches='tight')
