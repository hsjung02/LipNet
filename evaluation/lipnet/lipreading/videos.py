...
class Video(object):
    ...
    def process_frames_face(self, frames):
        ...
        #print(self.mouth)
        if np.array(mouth_frames).shape != (25, 50, 100, 3): #입 인식 안됐을때
            temp = []
            for i in range(len(mouth_frames)):
                temp.append(cv2.resize(mouth_frames[i],(100,50),interpolation=cv2.INTER_AREA))
            self.mouth = np.array(temp)
            self.set_data(temp)
        #print(self.mouth.shape)
        else:  #입 인식 됐을때
