...
def tokenize(text):
    return re.findall(r"\w+|[^\w\s]", text) #return re.findall(r"\w+|[^\w\s]", text, re.UNICODE)
...
class Spell(object):
    ...
    def sentence(self, sentence):
        ...
        return sentence #return unicode(sentence)
    '''
    def __init__(self, path):
        self.dictionary = Counter(list(string.punctuation) + self.words(open(path, 'rt', encoding='UTF8').read()))
